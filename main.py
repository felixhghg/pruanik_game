...
ни кто иной, как
АНОНИМУС!''',self.turn],['''Его глаза блеснули под маской...''',False],['''Он был настроен точно не позитивно.''',False],['''управление персонажем через WASD, 
взять/положить на подставку пряник через E,
бросить через q,рестарт через r''',False]]
        self.images = [PhotoImage(file='assets/textures/cutscene/frame1.png').zoom(8,8),PhotoImage(file='assets/textures/cutscene/frame2.png').zoom(8,8),PhotoImage(file='assets/textures/cutscene/frame3.png').zoom(8,8),PhotoImage(file='assets/textures/cutscene/frame3.png').zoom(8,8),PhotoImage(file='assets/textures/cutscene/frame4.png').zoom(8,8),PhotoImage(file='assets/textures/cutscene/frame5.png').zoom(8,8),PhotoImage(file='assets/textures/cutscene/frame5.png').zoom(8,8),PhotoImage(file='assets/textures/cutscene/frame6.png').zoom(8,8),PhotoImage(file='assets/textures/cutscene/frame7.png').zoom(8,8),PhotoImage(file='assets/textures/cutscene/frame6.png').zoom(8,8),PhotoImage(file='assets/textures/cutscene/frame7.png').zoom(8,8)]
        self.number = 0
...
class Level:
    ...
    class Pruanoprinimatel(Entities):
        ...
        def __init__(self, x, y, canvas):
            ...
            self.placement(canvas)
    # шипы
    class kover(Entities):
        texture = PhotoImage(file='assets/textures/entities/blocks/kover.png').zoom(8,8)
        def __init__(self, x, y, canvas):
            self.isnt_solid = True
            self.x = x
            self.y = y
            self.type = "kover"
            self.placement(canvas)
        ...
        for self.i_y in range(0,len(self.entities_text)):
            for self.i_x in range(0, len(self.entities_text[self.i_y])):
                if self.entities_text[self.i_y][self.i_x] == "#":
                    ...
                    self.entities[self.i_y].append(self.kover(self.i_x,self.i_y,self.c))
